...
gg= (py0**2+1)**0.5
alpha=10.0
b0=2*(alpha*py0*(1-np.cos(15.0/180.0*np.pi)))**0.5*1 #0.1 is for ratio
...
gg=(px**2+py**2+1)**0.5
alpha=10.0
b0=2*(alpha*p0*(1-np.cos(15.0/180.0*np.pi)))**0.5*1 #0.1 is for ratio
insert2='./in_15_grad/'
...
print(to_t)
to_t_2=to_t[(to_theta>-16)&(to_theta<-14)]
print(to_t_2)
...
photon_px=photon_px[(photon_theta>-16)&(photon_theta<-14)]
photon_py=photon_py[(photon_theta>-16)&(photon_theta<-14)]
photon = (photon_px**2+photon_py**2)**0.5
chi   = photon*b0/2.0/4.1e5
eta   = gg*b0/4.1e5
dt    = t0/6.28
...
for i in range(np.size(grid_x)):
    ...
    grid_y[i] = grid_y[i]*(2*xi[i]*gg*3**0.5/(4*pi**2)*kv(0.66666666666667,xi[i])**2)/result[0]
#plt.bar(grid_x, grid_y, width=0.3*grid_x, color='blue',alpha=0.5,edgecolor='black',linewidth=2,label='Monte-carlo method')
plt.plot(grid_x,grid_y,marker='o',markerfacecolor='tomato',markeredgecolor='darkred',markersize=10,markeredgewidth=3,label=r'$\theta_d=15.000^\circ$',linestyle='None')

print(to_t)
to_t_2=to_t[(to_theta>-14.5)&(to_theta<-12.5)]
print(to_t_2)
t0=(to_t_2[-1]-to_t_2[0])*2.0 #14.11078 #np.loadtxt(insert1+'t_0000.txt')
insert_n='_0000'
photon_px=np.loadtxt(insert1+'photon_px'+insert_n+'.txt')
photon_py=np.loadtxt(insert1+'photon_py'+insert_n+'.txt')
photon_theta=np.arctan2(photon_py,photon_px)/np.pi*180.
photon_px=photon_px[(photon_theta>-14.5)&(photon_theta<-12.5)]
photon_py=photon_py[(photon_theta>-14.5)&(photon_theta<-12.5)]
photon = (photon_px**2+photon_py**2)**0.5
chi   = photon*b0/2.0/4.1e5
eta   = gg*b0/4.1e5
dt    = t0/6.28
...
for i in range(np.size(grid_x)):
    ...
    grid_y[i] = grid_y[i]*(2*xi[i]*gg*3**0.5/(4*pi**2)*kv(0.66666666666667,xi[i])**2)/result[0]
#plt.bar(grid_x, grid_y, width=0.3*grid_x, color='blue',alpha=0.5,edgecolor='black',linewidth=2,label='Monte-carlo method')
plt.plot(grid_x,grid_y,marker='o',markerfacecolor='skyblue',markeredgecolor='darkblue',markersize=10,markeredgewidth=3,label=r'$\theta_d=13.500^\circ$',linestyle='None')

print(to_t)
to_t_2=to_t[(to_theta>-11)&(to_theta<-9)]
print(to_t_2)
t0=(to_t_2[-1]-to_t_2[0])*2.0 #14.11078 #np.loadtxt(insert1+'t_0000.txt')
insert_n='_0000'
photon_px=np.loadtxt(insert1+'photon_px'+insert_n+'.txt')
photon_py=np.loadtxt(insert1+'photon_py'+insert_n+'.txt')
photon_theta=np.arctan2(photon_py,photon_px)/np.pi*180.
photon_px=photon_px[(photon_theta>-11)&(photon_theta<-9)]
photon_py=photon_py[(photon_theta>-11)&(photon_theta<-9)]
photon = (photon_px**2+photon_py**2)**0.5
chi   = photon*b0/2.0/4.1e5
eta   = gg*b0/4.1e5
dt    = t0/6.28
...
for i in range(np.size(grid_x)):
    ...
    grid_y[i] = grid_y[i]*(2*xi[i]*gg*3**0.5/(4*pi**2)*kv(0.66666666666667,xi[i])**2)/result[0]
#plt.bar(grid_x, grid_y, width=0.3*grid_x, color='blue',alpha=0.5,edgecolor='black',linewidth=2,label='Monte-carlo method')
plt.plot(grid_x,grid_y,marker='o',markerfacecolor='yellow',markeredgecolor='darkorange',markersize=10,markeredgewidth=3,label=r'$\theta_d=10.000^\circ$',linestyle='None')
...
fig.savefig(to_path+'wrap_grad_in15.png',format='png',dpi=160)
