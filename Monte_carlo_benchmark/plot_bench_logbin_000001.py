...
def pxpy_to_energy_qed(gamma, weight, eta_i):
    binsize = 100
    en_grid = np.logspace(np.log10(1e-5*eta_i**2),np.log10(0.5*eta_i),100)
    en_bin  = np.logspace(np.log10(1e-5*eta_i**2),np.log10(0.5*eta_i),101)
...
def pxpy_to_energy_rr(gamma, weight, eta_i):
    binsize = 100
    en_grid = np.logspace(np.log10(1e-5*eta_i**2),np.log10(1e1*eta_i**2),100)
    en_bin  = np.logspace(np.log10(1e-5*eta_i**2),np.log10(1e1*eta_i**2),101)
    en_value = np.zeros_like(en_grid)
    delta_bin = en_bin[1:]-en_bin[:-1] 
    for i in range(binsize):
        en_value[i] = sum(weight[ (en_bin[i]<=gamma) & (gamma<en_bin[i+1]) ])/delta_bin[i]
...
insert1='./bench_data_qed_eta000001/'
gg=np.loadtxt(insert1+'py_0000.txt')
b0=np.loadtxt(insert1+'bz_part_0000.txt')
t0=np.loadtxt(insert1+'t_0000.txt')
gg = gg[0]
...
insert1='./bench_data_rr_eta000001/'
gg=np.loadtxt(insert1+'py_0000.txt')
b0=np.loadtxt(insert1+'bz_part_0000.txt')
t0=np.loadtxt(insert1+'t_0000.txt')
gg = gg[0]
...
#plt.ylim(0,1)
plt.text(1e-4*eta**2,0.6,r'$\eta=0.00001$',fontdict=font)
...
fig.savefig('./plot_bench_logbin_eta=000001.png',format='png',dpi=160)
#plt.close("all")
