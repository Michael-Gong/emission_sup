...
#  plt.plot(gg, dI_dl,'-k',linewidth=4, label=r'$\frac{dW}{dl}$',zorder=0)
  ...
  qi     = q0*79
  mi     = m0*1836*189
  gg     = np.logspace(0,4,1001)
  dN_dl_sample  = np.zeros([1001,5001])
  dN_dl_sumph   = np.zeros(1001)
  for i in range(1001):
      en_photon = np.logspace(-4,np.log10(0.5),5001)
      result_temp = integrate.quad(lambda x: 1./(1+x)*np.log(2*gg[i]*x), 1.0/en_photon[-1]-1, 1.0/en_photon[0]-1)
      dN_dl_sumph[i] = result_temp[0]*ni*q0**4*qi**2/(4*pi*epsilon0)**3*(16.0/(3*v0**3*h_planck/2/pi))/(m0**2*v0**2)*length
#      for j in range(5001):
#              result_temp1 = integrate.quad(lambda x: 1./(1+x)*np.log(2*gg[i]*x), 1.0/en_photon[j]-1.0, 1.0/en_photon[0]-1.0)
#              dN_dl_sample[i,j] = ni*q0**4*qi**2/(4*pi*epsilon0)**3*(16.0/(3*v0**3*h_planck/2/pi))/(m0**2*v0**2)*length*result_temp1[0]/dN_dl_sumph[i]
  plt.subplot(2,2,4)
  plt.plot(gg,dN_dl_sumph,'-k',linewidth=4, label=r'$\frac{dN}{dl}$',zorder=0)
  #### manifesting colorbar, changing label and axis properties ####
  #plt.xlim(0,520)
  #plt.ylim(3e4,4e4)
  plt.xlabel('$\gamma$',fontdict=font)
  ...
  fig.savefig('./brem_spectrum_log_3.png',format='png',dpi=160)
  plt.close("all")
